...
const bool fancon::Util::validIter(const string::iterator &end, std::initializer_list<string::iterator> iterators) {
...
string fancon::Util::getPath(const string &path_pf, const string &hwmon_id, const bool prev_failed) {
  ...
  if (!exists(p) || prev_failed)
    p = string(fancon_path + hwmon_id + '/' + path_pf);  // fancon path
